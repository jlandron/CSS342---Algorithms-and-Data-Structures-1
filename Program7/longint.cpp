...
LongInt LongInt::operator+( const LongInt &rhs ) const{
	...
	while( !(left.isEmpty()) && !(right.isEmpty()) ){
		...
		carry  = ( (left.removeBack() - '0') + (right.removeBack() - '0') + carry ) / 10;
...
LongInt LongInt::operator-( const LongInt &rhs ) const{
	...
	// make temp negative LongInt and do addition
	...
	if ( this->negative && rhs.negative ) { //both negative
		if (tempL < tempR) { 		//check ABS(left) < ABS(right)
			left = rhs.digits;
			right = this->digits;   //if true, result is positive, swap left and right
			res->negative = false;
		}else {						
		...
		if (tempL < tempR) {      	//check ABS(left) < ABS(right)
			left = rhs.digits;
			right = this->digits;	//if true, result is negative, swap left and right
			res->negative = true;
		}else {
			left = this->digits;	//else, result is negative, do not swap
			right = rhs.digits;
