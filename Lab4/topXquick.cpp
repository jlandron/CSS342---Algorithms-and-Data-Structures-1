...
template <class Comparable>
void quicksort( vector<Comparable> &a, int first, int last, int topX ) {
  int pivotIndex;

  // if the range is out of a.size( ) - topX through to a.size( ) - 1
  // don't do anything.
  // write code here.
  // if(last > a.size() - topX){
  //   return;
  // }
  if(first < a.size() - topX){
    return;
  }

  if ( first < last ) {
    partition( a, first, last, pivotIndex );
    quicksort( a, pivotIndex + 1, last, topX );
    quicksort( a, first, pivotIndex - 1, topX );
  }
}

template <class Comparable>
void quicksort( vector<Comparable> &a, int topX ) {
  quicksort( a, 0, a.size( ) - 1, topX );
}
